...
export function box(message, title, options) {
  return `${boxen(
    [title || chalk.white('APP Message'), '', chalk.white(foldLines(message, 0, 0, maxCharsPerLine()))].join('\n'),
    {
      borderColor: 'white',
      borderStyle: 'round',
      padding: 1,
...
export function infoBox(message, title) {
  return box(message, title || chalk.green('✔ APP Success'), {
    borderColor: 'blue'
  });
}
...
export function warningBox(message, title) {
  return box(message, title || chalk.yellow('⚠ APP Warning'), {
    borderColor: 'yellow'
  });
}

export function errorBox(message, title) {
  return box(message, title || chalk.red('✖ APP Error'), {
    borderColor: 'red'
  });
}

export function fatalBox(message, title) {
  return errorBox(message, title || chalk.red('✖ APP Fatal Error'));
}
