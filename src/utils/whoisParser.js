...
const extractContact = (data, xRegexes) => {
  const result = {};
  for (let i = 0; i < xRegexes.length; i += 1) {
    const match = XRegExp.exec(data, xRegexes[i]);
    if (match) {
      contanctAttributes.forEach(attr => {
        if (match[attr]) {
  ...
  if (data) {
    data.split('\n').forEach(line => {
      Object.keys(grammarRegexes).forEach(key => {
        for (let i = 0; i < grammarRegexes[key].length; i += 1) {
          const match = grammarRegexes[key][i].exec(line);
          if (match) {
            if (result[key]) {
              if (typeof result[key] === 'string') {
                result[key] = [result[key]];
              }
              result[key].push(match[1].trim());
            } else {
              result[key] = match[1] && match[1].trim();
            }
            break;
          }
        }
